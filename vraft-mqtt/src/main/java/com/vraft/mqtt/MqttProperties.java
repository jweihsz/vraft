...
/**
 * @author jweihsz
 * @version 2024/3/11 20:03
 **/
public final class MqttProperties {

    public MqttProperties() {this(true);}

    private MqttProperties(boolean canModify) {this.canModify = canModify;}
    ...
    public static final class UserProperties extends MqttProperty<List<StringPair>> {
        ...
        public void add(StringPair pair) {this.value.add(pair);}

        public void add(String key, String value) {this.value.add(new StringPair(key, value));}
    ...
    public static final class BinaryProperty extends MqttProperty<byte[]> {
        ...
        public BinaryProperty(int propertyId, byte[] value) {super(propertyId, value);}
    ...
    public void add(MqttProperty property) {
        String errMsg = null;
        if (!canModify) {
            errMsg = "adding property isn't allowed";
        ...
        if (property.propertyId == MqttPropertyType.USER_PROPERTY.value) {
            ...
            if (property instanceof UserProperty) {
                ...
                errMsg = "User property must be of UserProperty or UserProperties type";
            ...
            if (property instanceof IntegerProperty) {
                ...
                errMsg = "Subscription ID must be an integer property";
                throw new IllegalArgumentException(errMsg);
            }
        } else {
            if (props == null) {
    ...
    public Collection<? extends MqttProperty> listAll() {
        ...
        final int size = props != null ? props.size() : 1;
        List<MqttProperty> propValues = new ArrayList<MqttProperty>(size);
        if (props != null) {
    ...
    public MqttProperty getProperty(int propertyId) {
        ...
        if (propertyId == MqttPropertyType.SUBSCRIPTION_IDENTIFIER.value) {
            List<IntegerProperty> subscriptionIds = this.subscriptionIds;
            if (subscriptionIds == null || subscriptionIds.isEmpty()) {return null;}
